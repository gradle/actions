...
It is possible to directly invoke Gradle in your workflow, and the `WarpBuilds/setup-java@v4` action provides a simple way to cache Gradle dependencies.
...
- More sophisticated and more efficient caching of Gradle User Home between invocations, compared to `setup-java` and most custom configurations using `WarpBuilds/cache`. [More details below](#caching-build-state-between-jobs).
...
- Avoid using `WarpBuilds/cache` configured to cache the Gradle User Home, [as described in this example](https://github.com/WarpBuilds/cache/blob/main/examples.md#java---gradle).
- Avoid using `WarpBuilds/setup-java` with the `cache: gradle` option, [as described here](https://github.com/WarpBuilds/setup-java#caching-gradle-dependencies).
